...
NGLScene::NGLScene()
{
  ...
  m_spinXFace=0;
  m_spinYFace=0;
  setTitle("Particle Factory");

  m_emitterPos.set(0, 0, 0);
  m_emitterDir_Theta = 45;
  m_emitterDir_Phi = 45;
}
...
void NGLScene::paintGL()
{
  ...
  m_text->renderText(10,50,QString("z,x,c,v change emit angle of sphere"));
...
void NGLScene::addParticles()
{
    ...
    ngl::Vec3 dir;
    float theta = ngl::radians(m_emitterDir_Theta);
    float phi = ngl::radians(m_emitterDir_Phi);
    dir.m_x=sin(theta)*cos(phi);
    dir.m_y=sin(theta)*sin(phi);
    dir.m_z=cos(theta);
    dir *=0.5;
    m_scene->addParticle(m_emitterPos, dir, rng->getRandomColour(), 1, 800); // emitterPos, initialSpeed, colour, radius, life-how many iterations to die
...
void NGLScene::keyPressEvent(QKeyEvent *_event)
{
  ...
  case Qt::Key_Z : updateEmitterDir(-30, 0); break;
  case Qt::Key_X : updateEmitterDir(+30, 0); break;
  case Qt::Key_C : updateEmitterDir(0, -30); break;
  case Qt::Key_V : updateEmitterDir(0, +30); break;
  case Qt::Key_Space : clearParticles(); break;
...
void NGLScene::updateEmitterDir(float _d_theta, float _d_phi)
{
    m_emitterDir_Theta += _d_theta;
    m_emitterDir_Phi += _d_phi;
}
