...
class Scene
{
  public :
    /// @brief ctor
    /// @param[in] _t a pointer to the global transform
    ...
    void addParticle(ngl::Vec3 _pos, ngl::Vec3 _dir, ngl::Colour _c, GLfloat _r, GLfloat _ml);
  ...
  private :
    ...
    /// @brief the global transform,  used for global rotation only so could be replaced with
