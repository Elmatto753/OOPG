...
void NGLScene::addParticles()
{
    ...
    for(float i=-9.3;i<2; i+=2)
    {
        emitterPos.m_x = i;
        for(float j=-9.3;j<2; j+=2)
        {
            emitterPos.m_y = j;
            for(float k=-9.3;k<2; k+=2)
            {
                emitterPos.m_z = k;
                dir=rng->getRandomVec3();
                dir *=0.1;
                m_scene->addParticle(emitterPos, dir, rng->getRandomColour(), 0.2); // emitterPos, initialSpeed, colour, radius,
            }
        }
    }
}
...
void NGLScene::keyPressEvent(QKeyEvent *_event)
{
  ...
  case Qt::Key_Space : m_scene->clearParticles(); break;
  default : break;
  }
